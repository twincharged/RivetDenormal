...
|-GeoLoc
|-Notifs
|-Background jobs (Resque+Goworker?)
|
|*Web App
...
| + redcount(:field)
|
|-The only difference between modules is that RedArrayMethods does not
| need a reload_array method. The reload_array method is partially
| replaced by redget. The redget method exists because many of the
| foreign ids in Redis only need to be foreign ids and will not be
| translated into objects often. For instance: every time a new page
| is loaded for a user, each array of items will be scanned for a user
